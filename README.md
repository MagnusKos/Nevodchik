...
The bot-broker for LoRa mesh networks.

### What a strange name...

**Ne ́vodchik** - from the old colloquial Russian word "не́водчик": a fisherman who catches fish with a seine ("невод").
So, a mesh, a seine. Both are latticed. Get it? Me neither...

### What is it for?

This application allows you to connect to an MQTT broker, format received packets with user messages in human-readable format, and send them to IRC and/or to a connected Telegram bot.
...
First things first, setup the internal MQTT-client and the TG-bot provider: specify the connection parameters to your server! So either edit `nevodchik.conf` or pass env-vars (replace with your values, ofc):
* MQTT_HOST="localhost"
* MQTT_PORT=1883
* MQTT_USER="user"
* MQTT_PASSW="pass"
...
Configuration priorities (left one overrides right one):
**Args -> Env-vars -> Config-files -> In-app defaults**

Current state: *Very rough*
---------------------------
